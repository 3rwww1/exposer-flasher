...
module.exports = function (sockets, tree) {
  ...
  function refreshTimelaps(speed, zoom) {
    ...
    tree.get('program').forEach(function (exp, i) {
      ...
      var expCapturePath = exp.path + '/captures/';
      var expCaptures = glob.sync(expCapturePath + '/*/');

      if ((expCaptures.length > 0) && (i == tree.get('expo', 'id'))) {
        var lastCapture = _.last(_.sortBy(expCaptures, (val) => {
          return parseInt(val);
        }));
    ...
    if (lastImage !== null) {
      var movie = new ffmpeg();
        movie.addInput(lastImage)
        console.log('FFMPEG transcoding start...');
        movie.withFps(25)
          ...
          .on('end', function () {
            console.log('FFMPEG transcodning OK !');
            ...
            if (fs.existsSync('public/live.mp4')) {
              ...
              concat
                ...
                .on('error', function (err) {
                  console.error('an error happened during FFMPEG concatenation: ', err);
                })
                .on('end', function () {
                  console.log('FFMPEG concat OK !');
                  fs.unlinkSync('public/live.tmp.mp4');
                  fs.renameSync('public/live.concat.mp4', 'public/live.mp4');
                  console.log('live.mp4 updated')
                })
                .saveToFile('public/live.concat.mp4');
            } else {
              console.log('creating live.mp4 file')
              fs.renameSync('public/live.tmp.mp4', 'public/live.mp4');
            }
          })
          .on('error', function (err) {
            console.error('an error happened during FFMPEG transcode', err);
          })
          .saveToFile('public/live.tmp.mp4');
    } else {
      console.log('FFMPEG: nothing to transcode for now');
    }
  };
